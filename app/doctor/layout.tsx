...
import { doctorSignOut, useDoctorSession } from "@/lib/partner/auth";
...
export default function DoctorLayout({
  ...
  const isLogin = pathname.startsWith("/doctor/login");
  const router = useRouter();
  const { session, hydrated } = useDoctorSession();

  useEffect(() => {
    if (isLogin) return;
    if (hydrated && !session) router.replace("/doctor/login");
  }, [hydrated, session, isLogin, router]);

  if (isLogin) return <>{children}</>;
  if (!hydrated || !session) {
    return <div className="min-h-screen bg-[#f8fbf9]" />;
  }
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-8 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10.5px] text-[#0f3a26]/55">{session.username}</p>
            </div>
          </div>
          <button
